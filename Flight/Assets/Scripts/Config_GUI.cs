...
public class Config_GUI : MonoBehaviour
{
	public float volBG, volSFX, volATM, fov;
	public bool aa, shadows, sync, optionsGUI, fullscreen;
	public int res;
	public string settings, audiotype;
	public Rect optionsRect = new Rect(600, 300, 500, 500);
	...
	public void OptionsGUI(int gui)
	{
		...
		GUI.Label(new Rect(25, 125, 100, 30), "fullscreenScreen");
		fullscreen = GUI.Toggle(new Rect(95, 125, 100, 30), fullscreen, " On/Off");
		...
		if(GUI.Button(new Rect(25, 350, 100, 20), "Save Settings"))
			SaveAll();
		if(GUI.Button(new Rect(150, 350, 100, 20), "Back"))
			optionsGUI = false;
	...
	public void SaveAll()
	{
		...
		PlayerPrefs.SetInt("Custom_fullscreen", Convert.ToInt32(fullscreen));
	...
	public void LoadAll()
	{
		...
		fullscreen = Convert.ToBoolean(PlayerPrefs.GetInt("Custom_fullscreen"));
